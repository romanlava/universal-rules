...
## Triggers
Any operation whose result is not visible within the ladder's meaningful-feedback rung (see UX-ROBUST-02): form submissions, checkout payment processing, report or export generation, background jobs, provisioning or long-running setup tasks, dashboard data refresh, file uploads, heavy list or table rendering.
...
* Lock the triggering control against duplicate submission from the moment an operation starts until it resolves — canonical statement of this pattern. Lock it with `aria-disabled` plus a label naming the stage rather than a bare `disabled`, so it keeps focus and stays announceable; this in-flight lock is the only legitimate one, gating a control on invalid data is not (see UX-ERR-01).
...
### 🔴 DON'T → INSTEAD
* **Don't:** leave the interface visually frozen after a click with no acknowledgment. **Instead:** immediately switch the trigger control to a pending state that blocks duplicate submission until the operation resolves, kept focusable via `aria-disabled`.
...
✅ `<button aria-disabled="true">Processing payment…</button>` + "Contacting payment provider (usually ~10 s)" → button itself becomes "Order confirmed ✓" at the point of action
